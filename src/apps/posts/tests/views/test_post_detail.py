...
class PostDetailTest(APITestCase):
    ...
    def test_put_login_moderator(self):
        ...
        new_title = "This title is at least min length"
        self.assertNotEqual(new_title, self.public_post.title)
        res = self.client.put(
            self.public_url,
            {"title": new_title, "tags": self.tags},
        )
        ...
        self.public_post.refresh_from_db()
        self.assertEqual(new_title, self.public_post.title)
    ...
    def test_patch_login_moderator(self):
        ...
        new_title = "This title is at least min length"
        self.assertNotEqual(new_title, self.public_post.title)
        res = self.client.patch(self.public_url, {"title": new_title})
        self.assertEqual(res.status_code, status.HTTP_200_OK)
